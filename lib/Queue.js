...
module.exports = function(Q){
	...
	Queue.prototype = {
		...
		next : function(){
			if(this.ing < this.max){
				...
				if(_Obj && !this.isStop){
					...
					o.ing += 1;
					try{
						var nextp = _Obj.fun.apply(null,_Obj.regs)	
					}catch(err){
						o.ing -= 1;
		...
		//插入数组处理
		,'addArray': function(arr,fun,jump,start){
			var parrs = []
			var o = this;
			for(var i=0;i<arr.length;i++){
				+function(){
					var _i = i;
					var _fun = function(){return fun(arr[_i],_i,arr)};
					var _p = jump ? o.unshift(_fun) : o.push(_fun);
					parrs.push(_p);
				}()
			}
		...
		,all: function(arr,jump,start){
			var parrs = [];
			for(var i = 0;i<arr.length;i++){
				+function(){
					var _i = i;
					var funobj = arr[_i];
					var _p = jump ? o.unshift(funobj[0],afunobj[1],funobj[2]) : o.push(funobj[0],afunobj[1],funobj[2]);
					parrs.push(_p);
				}()
			}
			var nextP = getPromise();
